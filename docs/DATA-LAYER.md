...
## Item Master ขยายฟิลด์ตามฟอร์มระบบเดิม (2026-09-11)

ผู้ใช้ส่งภาพหน้าจอฟอร์ม "โครงสร้างสินค้า" จากระบบ ERP เดิมมาให้ทำตาม (ชื่อไทย/
อังกฤษ, รหัส/เสริม/บาร์โค้ด, หน่วย+หน่วยย่อย, คลาส/ไทป์/กรุ๊ป, รายละเอียด, มิติ
4 ด้าน+หน่วย, ปริมาตร, เวลา process, น้ำหนักต่ำสุด/สูงสุด/ต่อชิ้น, จำนวนสูญเสีย,
วันที่เริ่ม/ถึง) ผู้ใช้ยืนยันให้ทำครบทุกฟิลด์และ apply schema เข้าฐานข้อมูลจริงเลย
(ไม่ต้องรอ approve แยก) — ยังไม่ทันได้ทดสอบ BOM/MO พื้นฐานตามคำแนะนำก่อนหน้า
แต่ผู้ใช้เลือกเปลี่ยนแผนเป็นขยาย Item Master ก่อน

**Schema**: เพิ่ม 32 คอลัมน์ใหม่ใน `mst_items` แบบ additive (ไม่กระทบคอลัมน์เดิม,
ตารางว่างตอน apply) — ไฟล์ `supabase/erp-core-schema-item-master-fields.sql`
apply เข้า ERP_MNP จริง (`esxcwfrnoizftulqnudh`) แล้ว มี CHECK constraint บน
`code`/`name`/`name_en` ที่ล็อกกฎ 3 ข้อจากป้ายเตือนในภาพต้นฉบับ (ห้ามขึ้นต้นด้วย
สัญลักษณ์, ห้ามมี `\`, ห้ามมี `'`) ตรวจ `get_advisors` (security) หลัง apply แล้ว —
ไม่มี finding ใหม่จากการเปลี่ยนแปลงนี้ (มีแต่ warning เดิมที่บันทึกไว้แล้วด้านบน)

**การตัดสินใจออกแบบที่ทำเอง** (ผู้ใช้ไม่ได้ระบุ ตัดสินใจตามหลักวิศวกรรมที่สมเหตุสมผล):
- `item_group` เป็น free text ไม่ผูก FK กับ `org_departments` เพราะรหัสแผนกจริง
  ที่ seed ไว้เป็น composite (เช่น `BG-PT`, `PK-ST`) ไม่ตรงชุด RB/GR/PT/BG/PK/ST/WH
  ที่ผู้ใช้อธิบายไว้แบบ flat
- ไม่ตั้ง NOT NULL ตามเครื่องหมาย * ในภาพ (เช่น เสริม, หน่วยย่อย, คลาส) ที่ระดับ DB
  — บังคับแค่ฝั่ง UI (`required` + CSS `.req`) เพื่อไม่ให้ปิดกั้นการทดสอบรอบแรก
  ถ้าผู้ใช้กรอกข้อมูลไม่ครบทุกช่องตามภาพ ยกเว้น `code`/`name` (มี NOT NULL เดิมอยู่แล้ว)
- ฟิลด์มิติ (ยาว/กว้าง/หนา/สูง/ขนาด) และน้ำหนัก (ต่ำสุด/ต่อชิ้น/สูงสุด) ใช้ตัวเลือก
  หน่วยกลาง "หน่วยมิติ"/"หน่วยน้ำหนัก" ตัวเดียวใช้ร่วมกันแทนที่จะมี dropdown หน่วย
  แยกทุกช่องเหมือนภาพต้นฉบับ (ลด clutter ของฟอร์ม) แต่ DB ยังเก็บ `_uom` แยกคอลัมน์
  ต่อฟิลด์ไว้เผื่ออนาคตต้องการหน่วยต่างกันจริง
- `used_count`/`revision` (ในภาพเป็นช่องอ่านอย่างเดียวแสดง Used/Revision) เก็บเป็น
  ตัวนับ default 0 **ยังไม่มี trigger อัปเดตอัตโนมัติ** — ไม่โชว์ในฟอร์ม "เพิ่มสินค้าใหม่"
  เพราะไม่มีประโยชน์ตอนสร้างรายการใหม่ (ค่าเริ่มต้นคือ 0 เสมอ) ต้องทำตอนมีฟีเจอร์
  แก้ไข/ดูรายการเดิม (ยังไม่ได้สร้าง — ตอนนี้มีแค่ "เพิ่มสินค้าใหม่" เหมือนเดิม)

**UI**: แก้ `production.html`/`production.js`, เพิ่ม `production.css` ใหม่ (fieldset
กลุ่มตามภาพ: ส่วนหลัก/หมวดหมู่/รายละเอียด/มิติ&น้ำหนัก/วันที่ใช้งาน + กล่องเตือน
กฎ 3 ข้อ) และเพิ่ม `production.css` เข้า allowlist ใน `server.js` (`publicFiles`)
ที่ลืมไว้ตอนแรกทำให้ไฟล์ตอบ 403 — เจอจากการทดสอบจริง ไม่ใช่เดา

**ทดสอบแล้ว**: โหลดหน้าแบบ headless (Playwright, chromium ที่ pre-install ใน
sandbox) ก่อนล็อกอิน ไม่มี `pageerror`/JS exception, ทุกไฟล์ตอบ 200 ถ่าย screenshot
ยืนยัน layout เรียบร้อย **ยังไม่ได้ทดสอบ insert จริงผ่าน UI** (sandbox บล็อก
`*.supabase.co` เหมือนเดิม — ดูข้อจำกัดด้านบน) ผู้ใช้ต้องทดสอบเองในเบราว์เซอร์จริง
รวมถึง "เพิ่มสินค้าใหม่" ฟอร์มใหม่นี้ด้วย ก่อนจะถือว่าใช้งานได้จริง

**ยังไม่ทำ** (นอกขอบเขตรอบนี้): ฟอร์มแก้ไข/ดู/ลบ item ที่มีอยู่แล้ว (ภาพต้นฉบับมีเมนู
"โครงสร้างสินค้า-แก้ไข/-ดู/-ไม่เก็บเข้าคลังใหม่/-เปลี่ยนหน่วย/-เปลี่ยนคราส/-มีการใช้งาน"
แยกกัน — รอบนี้ทำแค่ "-ใหม่" เท่านั้นตามที่ผู้ใช้ขอ)

## สร้าง BOM/Routing แรกของ item ในหน้าเดียวกับ Item Master (2026-09-12)

ผู้ใช้ไม่ต้องการกดไปหน้า "วาง BOM"/"วาง Routing" แยกต่างหากทุกครั้งที่สร้างสินค้า FG
ใหม่ — อยากกรอกวัตถุดิบที่ใช้และขั้นตอนผลิตพร้อมกับ Item Master เลย แล้วให้ฝ่ายขาย
ลงออเดอร์แล้วระบบคำนวณเองจากข้อมูลนั้น

**การตัดสินใจ**: นี่คือเรื่อง UX ไม่ใช่เรื่อง schema — ยังต้องมี `prod_boms`/`prod_bom_lines`
และ `prod_routings`/`prod_routing_steps` แยกตารางเหมือนเดิม (เหตุผลเดิม: 1 item มีวัตถุดิบ
ได้หลายรายการ, และ MO ต้อง snapshot `bom_id`/`routing_id` ที่ใช้ตอนเปิดงานไว้ไม่ให้กระทบ
ถ้าสูตรเปลี่ยนทีหลัง) เสนอผู้ใช้ 2 ทาง — (A) คง schema เดิม รวมแค่ UI ให้กรอกที่เดียว กับ
(B) ตัด versioning ทิ้งจริง — ผู้ใช้เลือก **(A)**

**สิ่งที่ทำ**: `production.html`/`production.js` — ฟอร์ม "เพิ่มสินค้าใหม่" (เฉพาะตอนเลือก
ประเภท FG) มี fieldset ใหม่ 2 อัน ให้เพิ่ม/ลบแถว "วัตถุดิบที่ใช้" (component + จำนวนต่อหน่วย
+ สูญเสีย % + แผนก) และ "ขั้นตอนการผลิต" (แผนก + เป้าหมาย/ชั่วโมง + เวลาติดตั้ง — ลำดับ Seq
กำหนดอัตโนมัติเป็น 10, 20, 30... ตามลำดับแถวที่กรอก ไม่ต้องพิมพ์เอง) ตอนกด "บันทึก" จะ insert
item ก่อน แล้วถ้ามีแถววัตถุดิบ/ขั้นตอนกรอกไว้ จะสร้าง `prod_boms` เวอร์ชัน `BOM-01` และ/หรือ
`prod_routings` เวอร์ชัน `R-01` พร้อมแถวที่กรอกให้อัตโนมัติ (สาม insert แยกกันฝั่ง browser
ไม่ใช่ transaction เดียว — ช่องโหว่เดิมของโมดูลนี้จนกว่าจะมี service layer จริง ถ้าขั้นกลางพัง
สถานะข้อความจะบอกว่าไปพังขั้นไหน ให้ไปแก้ผ่านการ์ด "วาง BOM"/"วาง Routing" เดิมได้)

การ์ด "วาง BOM"/"วาง Routing" เดิมยังอยู่ — ใช้ตอนต้องการแก้ไขสูตรเดิมหรือเพิ่มเวอร์ชันใหม่
ทีหลัง (สิ่งที่ฟอร์มสร้างสินค้าใหม่ทำแทนไม่ได้ เพราะสร้างได้แค่เวอร์ชันแรกตอนสร้าง item)

**ทดสอบแล้ว**: โหลดหน้าแบบ headless (signed out เหมือนเดิม, sandbox บล็อก
`*.supabase.co`) ไม่มี console error ใหม่ ปุ่มเพิ่ม/ลบแถวและการซ่อน/แสดง fieldset ตอนสลับ
